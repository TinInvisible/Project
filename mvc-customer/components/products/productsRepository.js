...
exports.sortByNameDesc = async() =>{
  const result = await db.connection.execute("select * from productdetail order by productdetail.Pr DESC");
  return result[0];
}  
...
exports.sortByNameASC = async() =>{
  const result = await db.connection.execute("select * from productdetail order by productdetail.Pr ASC");
  return result[0];
}  
